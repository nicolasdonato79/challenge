...
public class SearchSpecifications<T> implements Specification<T> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4707747614015495126L;
	...
	public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		...
		for (SearchCriteria criteria : searchCriteriaList)
			...
			else if (criteria.getSearchOperation().equals(SearchOperation.LIKE))
				predicates.add(builder.like(builder.lower(root.get(criteria.getAttr())),
						( "%"+ criteria.getValue().toString().trim().toLowerCase()+ "%")));
			else if (criteria.getSearchOperation().equals(SearchOperation.LIKE_END))
				predicates.add(builder.like(builder.lower(root.get(criteria.getAttr())),
						criteria.getValue().toString().toLowerCase() + "%"));
			else if (criteria.getSearchOperation().equals(SearchOperation.LIKE_START))
				predicates.add(builder.like(builder.lower(root.get(criteria.getAttr())),
						"%" + criteria.getValue().toString().toLowerCase()));
